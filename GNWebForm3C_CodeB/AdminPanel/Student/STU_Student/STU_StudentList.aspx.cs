...
public partial class AdminPanel_Student_STU_Student_STU_StudentList : System.Web.UI.Page
{
    #region 11.0 Variables

    String FormName = "STU_StudentList";
    ...
    private void Search(int PageNo)
    {
        ...
        SqlString StudentName = SqlString.Null;
        SqlString EnrollmentNo = SqlString.Null;
        SqlInt32 CurrentSem = SqlInt32.Null;
        SqlString EmailInstitute = SqlString.Null;
        SqlString EmailPersonal = SqlString.Null;
        SqlString Gender = SqlString.Null;
        SqlInt32 RollNo = SqlInt32.Null;
        SqlString ContactNo = SqlString.Null;
        ...
        if (txtStudentName.Text.Trim() != String.Empty)
            StudentName = Convert.ToString(txtStudentName.Text.Trim());
        if (txtEnrollmentNo.Text.Trim() != String.Empty)
            EnrollmentNo = Convert.ToString(txtEnrollmentNo.Text.Trim());
        ...
        if (txtEmailInstitute.Text.Trim() != String.Empty)
            EmailInstitute = Convert.ToString(txtEmailInstitute.Text.Trim());
        if (txtEmailPersonal.Text.Trim() != String.Empty)
            EmailPersonal = Convert.ToString(txtEmailPersonal.Text.Trim());
        ...
        if (txtContactNo.Text.Trim() != String.Empty)
            ContactNo = Convert.ToString(txtContactNo.Text.Trim());

        #endregion Gather Data

        STU_StudentBAL balSTU_Student = new STU_StudentBAL();

        DataTable dt = balSTU_Student.SelectPage(Offset, PageRecordSize, out TotalRecords, StudentName, EnrollmentNo, CurrentSem, EmailInstitute, EmailPersonal, Gender, RollNo, ContactNo);
    ...
    protected void rpData_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        if (e.CommandName == "DeleteRecord")
        {
            try
            {
                ...
                {
                    if (balSTU_Student.Delete(Convert.ToInt32(e.CommandArgument)))
                    {
    ...
    protected void lbtnExport_Click(object sender, EventArgs e)
    {
        ...
        if (txtStudentName.Text.Trim() != String.Empty)
            StudentName = Convert.ToString(txtStudentName.Text.Trim());
        if (txtEnrollmentNo.Text.Trim() != String.Empty)
            StudentName = Convert.ToString(txtEnrollmentNo.Text.Trim());
        if (ddlCurrentSem.SelectedIndex > 0)
            CurrentSem = Convert.ToInt32(ddlCurrentSem.SelectedValue);
        if (txtEmailInstitute.Text.Trim() != String.Empty)
            EmailInstitute = Convert.ToString(txtEmailInstitute.Text.Trim());
        ...
        DataTable dtSTU_Student = balSTU_Student.SelectPage(Offset, PageRecordSize, out TotalReceivedRecord, StudentName, EnrollmentNo, CurrentSem, EmailInstitute, EmailPersonal, Gender, RollNo, ContactNo);
        if (dtSTU_Student != null && dtSTU_Student.Rows.Count > 0)
        {
            Session["ExportTable"] = dtSTU_Student;
